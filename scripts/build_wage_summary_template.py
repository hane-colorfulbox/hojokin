...
- R216 = Σ(対象者の[Σ月次総支給 − 非課税通勤手当年額 + 課税賞与年額])
...
- 通勤手当の月割減算で月が0円化する超レアケースはツール＝丸ごと除外／本様式＝年額
  差引のまま算入（公募要領は月割配賦を規定していないため様式側の挙動を正とする）。
...
def build_help_sheet(ws):
    ...
    lines = [
        ...
        ('・給与ソフト等で各月の「課税支給合計（課税計）」が分かる場合は、そちらの方が正確です。', F_SMALL, None),
        ('　その場合は「年間通勤手当」欄を空欄にしてください（二重に差し引かないため）。', F_SMALL, None),
        ...
        ('　（補助金のルールで非課税の通勤手当は集計から除く決まりです。空欄のままだと', F_SMALL, None),
        ('　総額が過大になり、賃上げ目標が実態より高く設定されてしまいます。金額は', F_SMALL, None),
        ('　給与ソフトの「非課税支給額」や賃金台帳の通勤手当欄でご確認いただけます）', F_SMALL, None),
