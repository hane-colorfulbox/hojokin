...
INCLUDE_FILES = [
    ...
    # 制度そのもの（枠・加点マトリクス・対象者・スケジュール・採択実績）の一次情報カタログ。
    # 出典ラベル付き＝「人から聞いた話」と「一次資料で確認した事実」を分けてある。脱PII。
    'docs/補助金_制度リファレンス.md',
    # 上記の鮮度管理。公式資料の更新を検知する（標準ライブラリのみ・API課金なし）。
    'docs/制度一次情報/sources.json',
    'scripts/check_seido_freshness.py',
