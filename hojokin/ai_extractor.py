...
7. 給与と賞与が別行・別シートに分かれている場合は、同月分を**合算**してください。
   ...
   - **★ 月度コードが「YYYYMM」形式（例: 202521, 202522）で月部分が 13 以上の場合**: これは「賞与1（21）」「賞与2（22）」を意味する給与ソフト独自表記。**ただし給与ソフトによって慣習が異なる**（21=春賞与/22=夏賞与のソフトもある）ため、**支給日（paid_date）列が同じ行に存在するならその実支給月を優先**してマッピングすること。paid_date が無い場合のフォールバックとして、202521→7月（Index 6）、202522→12月（Index 11）に加算する
...
7. 給与と賞与が別行・別シートに分かれている場合は、同月分を**合算**してください。
   ...
   - **★ 月度コードが「YYYYMM」形式で月部分が 13 以上**（例: 202521, 202522）: 賞与の給与ソフト独自表記。**支給日（paid_date）列が同じ行に存在するならその実支給月を優先**。paid_date が無いときのみフォールバックとして 202521→7月、202522→12月にそれぞれ加算
   - 賞与シートが見つからない場合は、PL の「賞与手当」科目との突合で漏れを検出するため、可能な限り全シートを探索する
8. 月の判定は以下のいずれかを使用:
   - 列ヘッダ「1月」「5月」等のプレーン表記
   - 「R6.5月」「R7.4月」「令和6年5月」等の和暦付き表記（年は無視して月だけ使用）
