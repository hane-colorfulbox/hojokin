...
7. 給与と賞与が別行・別シート・**別ページ**に分かれている場合は、同一従業員に**必ず合算**してください。
   ...
   - **★★★ 個人別連続ページ型 PDF の処理（給与計算ソフト出力でよくあるパターン）★★★**
     ...
     **同一従業員の月給と賞与が連続する別ページに分かれて記載される** PDF があります。
     ...
     - 賞与ページに「賞与1回」「賞与2回」「夏季賞与」「冬季賞与」「期末賞与」等の列・行があれば、各賞与の**支給日**（例: 令和7年8月25日, 令和8年2月27日）から該当する**カレンダー月**を判定し、その月の monthly_wages に加算する
       - 支給日が「令和7年8月25日」→ 8月 → Index 7 に加算
       - 支給日が「令和8年2月27日」→ 2月 → Index 1 に加算
       - **★ 賞与の支給日が月給ページに記載がない月（例: 2月のみ賞与で月給なし）でも、その月の monthly_wages にその賞与額を入れる**（null のままにしない）
     - **賞与ページの「総支給額(課税)」または「賞与額」を、月給ページの該当月の「総支給額(課税)」に加算**する。差引支給額や控除後の金額は使わない
     - 同一人物の賞与を**取りこぼさないこと**: 賞与1回・賞与2回の両方を必ず加算する。片方だけ取り込んで他方を捨てるのは禁止
   - 賞与は通常「夏季賞与（6月・7月・8月）」「冬季賞与（12月）」「期末賞与（決算期月）」として支給される。該当月の monthly_wages = 月給 + 賞与 で合算する
   - 「賞与」「ボーナス」「期末手当」「特別手当」を含むシート名・列名・行名・**ページ見出し** は **絶対に見落とさない**こと
   - 賞与シートが見つからない場合は、PL の「賞与手当」科目との突合で漏れを検出するため、可能な限り全シート・全ページを探索する
8. 月の判定は以下のいずれかを使用:
   - 列ヘッダ「1月」「5月」等のプレーン表記
   - 「R6.5月」「R7.4月」「令和6年5月」等の和暦付き表記（年は無視して月だけ使用）
   - 「2024年5月」「2024/05」「202405」等の西暦表記
   - 「対象年月」「給与年月」列の値
9. **【月給制の labor_hours 出力ルール】** 正社員・役員は通常「月給制」で、賃金台帳に労働時間欄がないか、勤怠記録（残業時間・休日時間等）のみが部分的に書かれているケースがあります。
