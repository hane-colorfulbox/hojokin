...
PROMPT_PL = """この損益計算書と販管費内訳書の画像から、以下をJSON形式で抽出してください。
...
class ClaudeExtractor(BaseExtractor):
    ...
    def generate_ai_judgment(self, company, financial, tool_name, hearing_data=None) -> AIJudgment:
        # ヒアリングデータからIT投資関連情報を取得
        it_investment_answer = '不明'
        it_investment_amount = '不明'
        it_investment_process = '不明'
        if hearing_data:
            # 行74: IT投資年間金額, 行75: IT投資プロセス（ヒアリングシートの行番号）
            for row_num, item in hearing_data.items():
                label = str(item.get('label', ''))
                value = item.get('value')
                if 'IT投資' in label and '金額' in label:
                    it_investment_answer = 'はい' if value else 'いいえ'
                    it_investment_amount = str(value) if value else 'なし'
                elif 'IT投資' in label and 'プロセス' in label:
                    it_investment_process = str(value) if value else 'なし'
        ...
        prompt = PROMPT_AI_JUDGMENT.format(
            ...
            tool_name=tool_name,
            it_investment_answer=it_investment_answer,
            it_investment_amount=it_investment_amount,
            it_investment_process=it_investment_process,
        )
