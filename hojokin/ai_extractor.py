...
5. employment_type: 雇用形態（正社員・パート・アルバイト・役員等）。元の表記をそのまま入れてください。役員は「役員」を含む表記に。
   - **★ 雇用形態が台帳に明示されていない場合は `"正社員(推定)"` を既定値とする**（null/空文字列にしない）。
     後段の wage_validator.check_employment_type_missing が「(推定)」を含む値を「人間チェック対象」として警告するため、
     必ず「正社員(推定)」表記で出力すること（「正社員」だけにすると provenance が失われる）
6. **【名前抽出の厳密ルール】**
...
7. **【月給データの月配置ルール — 最重要】**
   monthly_wages[i] の Index は **賃金台帳に書かれた「○月分」「○月度」のラベル（勤務月）** で決定する。
   - 「3月分」「3月度」「R7.3月」等のラベル → **Index 2（3月）に格納**
   - 「11月分」「11月度」 → Index 10
   - 「1月分」 → Index 0
   - **★ 支給日（例: 「令和7年4月10日支給」）は使わない**: 賃金台帳の「○月分」ラベルが「4月10日に支給された」と書かれていても、ラベル「3月分」なら **Index 2（3月）** に格納する。支給日でカレンダー月を決めない
   - 「3月分」とラベル付けされた金額を「4月分」のIndexに入れると、月配置が連鎖的にズレて全データが破壊される。これは**絶対NG**
   - **★★ 同一PDF内でも従業員ごとに月ラベル範囲が異なる場合があります（給与計算ソフトの支給日基準集計では一般的）**
     - 例1: 一般社員は「3月分〜1月分」(11ヶ月)、取締役は「2月分〜1月分」(12ヶ月) のように、入社時期・役職で範囲が違う
     - 例2: 中途入社者・期中退社者が混在し、それぞれ別のラベル範囲を持つ
     - **各従業員ごとに、その従業員の行/ページの月ラベルだけを見て独立判定する**こと
     - PDF全体や多数派の従業員のパターンに合わせて、他の従業員の範囲を強制的に揃えてはいけない（例: 大多数が11ヶ月だからといって、12ヶ月ラベルの取締役の最初の月を捨てない）
     - 各従業員のページ/行のラベル数とその範囲を、必ずそのまま全部取り込む

8. **【賞与データの月配置ルール】**
   賞与ページに「賞与1回」「賞与2回」「夏季賞与」「冬季賞与」「期末賞与」等の記載があれば、各賞与の**支給日**から該当する**カレンダー月**を判定し、その月の monthly_wages に**加算**する。
   - 賞与の場合は「○月分」というラベルがないため、**支給日で月を決める**のが正解
   - 支給日が「令和7年8月25日」→ 8月 → Index 7 に加算
   - 支給日が「令和8年2月27日」→ 2月 → Index 1 に加算
   - **★ 賞与の支給日に対応する月が月給ページに記載がない場合（例: 2月のみ賞与で月給なし）でも、その月の monthly_wages にその賞与額を入れる**（null のままにしない）
   - **賞与ページの「総支給額(課税)」または「賞与額」を、月給ページの該当月の「総支給額(課税)」に加算**する。差引支給額や控除後の金額は使わない
   - **同一人物の賞与を取りこぼさないこと**: 賞与1回・賞与2回の両方を必ず加算する。片方だけ取り込んで他方を捨てるのは禁止
   - **★★ 典型的な賞与レイアウト ★★** PDFに「賞与1回 賞与2回 合計」という3列ヘッダがあり、その下に「支給日（令和7年8月25日、令和8年2月27日）」「賞与額 100,000 300,000 400,000」のような表が現れることがある。この場合：
     - 「賞与1回 100,000円」（令和7年8月25日支給）→ Index 7（8月）の monthly_wages に **加算**
     - 「賞与2回 300,000円」（令和8年2月27日支給）→ Index 1（2月）の monthly_wages に **加算**
     - 「合計 400,000円」は無視（個別の賞与1回・2回を加算する）
     - 賞与1回も賞与2回も**両方必ず加算**。一方を選んでもう一方を捨てるのは絶対NG
     - その従業員の monthly_wages の該当月（8月・2月）に既存値があるか null かにかかわらず、賞与額を**加算（または新規セット）**する
   - 賞与は通常「夏季賞与（6月・7月・8月）」「冬季賞与（12月）」「期末賞与（決算期月）」として支給される
   - 「賞与」「ボーナス」「期末手当」「特別手当」を含むシート名・列名・行名・**ページ見出し** は **絶対に見落とさない**こと

9. **【個人別連続ページ型 PDF の処理（給与計算ソフト出力でよくあるパターン）】**
   1ページ目=Aさんの月給、2ページ目=Aさんの賞与、3ページ目=Bさんの月給、4ページ目=Bさんの賞与... のように、
   **同一従業員の月給と賞与が連続する別ページに分かれて記載される** PDF があります。
   - 各ページの冒頭の「氏名」を必ず確認し、**同一氏名（フリガナを含む完全一致）の連続ページは同一人物として monthly_wages を統合**してください
   - 「賞与一覧シート」「賞与明細シート」「ボーナス一覧」等の名称で別タブに賞与情報がある場合も同様に、対応する従業員の monthly_wages にその月の賞与額を加算する
   - 賞与ページが見つからない場合は、PL の「賞与手当」科目との突合で漏れを検出するため、可能な限り全シート・全ページを探索する
10. 月の判定は以下のいずれかを使用:
   - 列ヘッダ「1月」「5月」等のプレーン表記
   - 「R6.5月」「R7.4月」「令和6年5月」等の和暦付き表記（年は無視して月だけ使用）
   - 「2024年5月」「2024/05」「202405」等の西暦表記
   - 「対象年月」「給与年月」列の値
11. **【月給制の labor_hours 出力ルール】** 正社員・役員は通常「月給制」で、賃金台帳に労働時間欄がないか、勤怠記録（残業時間・休日時間等）のみが部分的に書かれているケースがあります。
   - 「総労働時間」「実労働時間」「所定労働時間」**列が明示的に存在しない** 場合、monthly_hours は **全12要素を null** にしてください
   - 「平日普通」「休日普通」「時間外」「残業」「深夜」等の **内訳時間しかない** 場合も、合算・推測は禁止 → monthly_hours は null
   - 「出勤日数 × 8時間」のような推測値も入れない → monthly_hours は null（後段が必要に応じて補完する）
   - employment_type に「正社員」または「役員」が含まれ、賃金台帳に時給列もない場合、**monthly_hours は全 null が正しい**（パートのみ時間を入れる）
...
- 同一従業員の月給ページと賞与ページが分離している PDF では、**必ず統合して 1 人 1 レコードにまとめる**こと（賞与だけ別レコードにしない）
...
4. monthly_work_days: 月別の **労働日数 / 出勤日数 / 勤務日数**。Index は monthly_wages と同じ規則。
   - 「有給休暇日数」は労働日数ではないので含めないこと。
5. employment_type: 雇用形態（正社員・パート・アルバイト・役員等）。元の表記をそのまま入れてください。役員は「役員」を含む表記に。
   - 雇用形態が台帳に明示されていない場合は `"正社員(推定)"` を既定値とする（後段検証で人間チェック対象として警告される）
6. **【名前抽出の厳密ルール】**
   - 賃金台帳の各行は通常、以下の列構成：フリガナ / 氏名 / 性別 / 生年月日 / 住所 / 給与・労働時間...
   - 氏名は『氏名』『社員名』『名前』など明示的なラベルが付いた欄 **からのみ** 抽出。決して隣接する「住所」「市町村」欄から文字を引っ張らない
   - 抽出後、名前に「市」「県」「区」「都」「道」「町」「村」など行政地名が含まれていないか確認。含まれていたら、その行の住所欄を参照して誤抽出部分を削除する
