    ...
    try:
        ...
        if wage_status == 'no_data':
            ...
            wage_warning = (
                ...
                'デジタル化・AI導入補助金 公募要領 p.10 では「従業員を雇用していない法人は、従業員を'
        ...
        # デジタル化・AI導入補助金 公募要領 p.10「パートタイム従業員については正社員の就業時間に換算」を満たすため、
        ...
        if wage_plan and wage_plan.get('part_fte_fallback_count', 0) > 0:
            ...
            wage_warning += (
                ...
                f'デジタル化・AI導入補助金 公募要領 p.10 は「正社員の就業時間に換算」を要求。'
        ...
        if wage_plan and wage_plan.get('excluded_bonus_only_count', 0) > 0:
            _b_names = wage_plan.get('excluded_bonus_only_names', []) or []
            _b_preview = ', '.join(_b_names[:3])
            if len(_b_names) > 3:
            ...
            wage_warning += (
                ...
                f'年間賞与も給与支給総額に算入していません）。'
                f'算入すべき事情がある場合は、支給実態のある月に賞与額を記入し'
                f'年間賞与欄から同額を差し引いて再実行してください'
            )
...
def _calc_wage_plan_from_ledger(
    ...
    try:
        ...
        if result.total_salary <= 0:
            ...
            if included_count == 0:
                ...
                if result.excluded_bonus_only_names:
                    zero_note += (
                        f'⑤賞与のみ受給（月次全月0円）の従業員'
                        f'{len(result.excluded_bonus_only_names)}名は算定対象外、'
                    )
                logger.warning(
        ...
        if result.excluded_bonus_only_names:
            ...
            plan['excluded_bonus_only_names'] = list(
                result.excluded_bonus_only_names
            )
            logger.info(
                f'賞与のみ受給（月次全月0円）のため算定対象外: '
                f'{len(result.excluded_bonus_only_names)}名 '
                f'({", ".join(result.excluded_bonus_only_names[:5])}'
                f'{"..." if len(result.excluded_bonus_only_names) > 5 else ""})'
            )
        logger.info(
            f'給与支給総額: {base:,.0f}円 '
        ...
        if part_fte_fallback_count > 0:
            logger.warning(
                ...
                f'デジタル化・AI導入補助金 公募要領 p.10 / マニュアル p.86 では「正社員就業時間に換算した小数値」を要求。'
        ...
        if low_full_year_ratio:
            ...
            bonus_only_n = len(result.excluded_bonus_only_names)
            # 除外集合は互いに素（partial は支給月あり、zero/bonus_only は全月0円明記で
            # 賞与の有無により排他）。引き忘れると中途入退社の人数が二重計上される
            midyear_n = (non_officer_count - included_count
                         - zero_n - partial_n - bonus_only_n)
            zero_part = f'・給与支給0円{zero_n}名' if zero_n else ''
            partial_part = f'・0円支給月あり{partial_n}名' if partial_n else ''
            bonus_only_part = f'・賞与のみ受給{bonus_only_n}名' if bonus_only_n else ''
            logger.warning(
                f'全月在籍者({included_count}名)が非役員数({non_officer_count}名)の'
                f'{FISCAL_MISMATCH_RATIO*100:.0f}%未満。中途入退社{midyear_n}名'
                f'{partial_part}{bonus_only_part}{zero_part}は公募要領通り算出対象から除外。'
...
def _build_employees_detail_from_ledger(
    ...
    for emp in employees:
        ...
        # 0円・負値・空欄の月はいずれも「支給を受けていない月」（公募要領 p.10）として
        # 在籍月に数えない（「賃金台帳の作成」産の台帳は不在月を0円で出力するため、
        # non-None 判定では中途入社者が full_year=True になり R215/R216 に誤算入。
        # v0.2.69）。全月0円＋賞与のみ受給者も除外（v0.2.70、2026-06-11 ルール確定）。
