...
def detect_prefecture(address: str) -> str | None:
    ...
    # 都道府県名なしの場合、市区町村名から推定（全47都道府県の県庁所在地 + 政令指定都市）
    CITY_TO_PREF = {
        # 政令指定都市・主要市
        ...
        # 残り県庁所在地（県名と被らないユニークな市名のみ。
        # 「水戸市」「宇都宮市」「前橋市」等は県名と全く違うので市名だけで判定可能）
        '青森市': '青森県', '盛岡市': '岩手県', '秋田市': '秋田県',
        '山形市': '山形県', '福島市': '福島県',
        '水戸市': '茨城県', '宇都宮市': '栃木県', '前橋市': '群馬県',
        '富山市': '富山県', '金沢市': '石川県', '福井市': '福井県',
        '甲府市': '山梨県', '長野市': '長野県',
        '岐阜市': '岐阜県', '津市': '三重県', '大津市': '滋賀県',
        '奈良市': '奈良県', '和歌山市': '和歌山県',
        '鳥取市': '鳥取県', '松江市': '島根県', '山口市': '山口県',
        '徳島市': '徳島県', '高知市': '高知県',
        '佐賀市': '佐賀県', '長崎市': '長崎県',
        '大分市': '大分県', '宮崎市': '宮崎県',
        # 注: 県名と同じ市名（千葉市/愛媛県松山市等）は上の都道府県名チェックでヒットするので
        #     ここに重複登録しても安全（最初のループで都道府県名検出が先に動く）
    }
    # 長いキーから順にチェック（「大津市」を「津市」より先に判定するため）
...
MAPPING_2026_TSUJO = TemplateMapping(
    ...
    kyuyo={
        ...
        'salary':           (5,  5),  # E5:  給料手当（E列=販管費 / F列=原価報告書に分割転記）
        'misc_wages':       (6,  5),  # E6:  雑給
        'bonus':            (7,  5),  # E7:  賞与手当
        # ※ 役員報酬はこのシートにマッピングしない。生産性指標シートの人件費ブロックは
        #   E4='販管費'/F4='原価報告書' のヘッダー行＋給料手当/雑給/賞与手当/旅費交通費の
        #   科目行のみで、役員報酬の行は存在しない（E5:E9 の人件費合計からも除外＝R216 と整合）。
        #   officer_comp を E4(4,5) に割り当てると『販管費』ヘッダーを上書き破壊するため外す。
...
MAPPING_2026_INVOICE_KOJIN = TemplateMapping(
    ...
    shinsei={
        ...
        # R45 現在住所（本人確認資料から。AI抽出は未対応のため担当者手書き）
        'headquarters_address': 45,
        'industry_code': 46,
        'industry_text': 47,
