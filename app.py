...
with st.expander(
    ...
    with _tab_full:
        st.markdown(
            ...
            '案件情報の `○○` を埋めて CC に貼り付け。\n\n'
            'この依頼文は **CC が補助金リポジトリにアクセスできない前提** で書かれています。'
            'テンプレファイルやパーサーが無くても、マニュアルの仕様だけで Excel を新規作成できます。'
        )
        if _WAGE_MANUAL_PATH.exists():
            ...
            _full_request = (
                ...
                '【前提】\n'
                'あなた（CC）は補助金リポジトリにアクセスできない環境で動いています。\n'
                '「ツール/賃金台帳テンプレート.xlsx」「hojokin/wage_reader.py」などのリポジトリ内ファイルは\n'
                '読めないものとして作業してください。代わりに、下のマニュアル §1「変換先テンプレート」の\n'
                'ヘッダー仕様に従って openpyxl で .xlsx を新規作成してください。\n\n'
...
# ── フッター ──
st.markdown('---')
st.caption(f'補助金書類自動作成ツール v0.2.3 | カラフルボックス株式会社')
