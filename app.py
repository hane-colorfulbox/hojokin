...
with st.expander(
    ...
    st.markdown(
        '各自のPCで CC（claude.ai のチャット / 自分の作業フォルダで起動した Claude Code 等）に'
        '賃金台帳の変換をしてもらうための依頼文です。'
        '補助金リポジトリのクローンが手元になくても、マニュアル本文を一緒に渡せばCCが作業できます。'
    )
    _tab_full, _tab_short = st.tabs([
        '【標準】全文コピペで依頼する（各自のPCで作業する人向け）',
        '【補助金リポジトリで CC を起動している人向け】パス参照（短縮版）',
    ])

    with _tab_full:
        st.markdown(
            '下のブロックを **丸ごとコピー**（右上のコピーボタン）→ 案件情報の `○○` を埋めて'
            'CC（claude.ai のチャット欄 or 手元のClaude Code）に貼り付けて送信してください。'
        )
        if _WAGE_MANUAL_PATH.exists():
            ...
            _full_request = (
                ...
                '  カレンダー月の位置に入れる（マニュアル §1.1 の落とし穴を必ず確認）\n\n'
                '【案件情報】\n'
...
# ── フッター ──
st.markdown('---')
st.caption(f'補助金書類自動作成ツール v0.2.4 | カラフルボックス株式会社')
